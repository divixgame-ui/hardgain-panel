...
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    body{background:#060608;color:#d0d0d8;font-family:${TENANT.font};-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;}
    ...
    input:focus,textarea:focus,select:focus{border-color:#FF6B3555!important;outline:none;}
...
const LEAD_S = {new:["Nowy","#FF6B35"],contacted:["Kontakt","#F7C59F"],qualified:["Kwalif.","#4ECDC4"],closed_won:["Zamknięty","#45B7AA"],closed_lost:["Odpada","#5a5a7a"]};
  ...
  return (
    <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #1a1a2e",borderRadius:16,padding:"18px 20px",position:"relative",overflow:"hidden"}}>
      ...
      {(sub||trend)&&<div style={{display:"flex",alignItems:"center",gap:6,marginTop:6}}>
        ...
        {sub&&<span style={{fontSize:10,color:"#666680"}}>{sub}</span>}
      </div>}
    </div>
  );
}
...
const SH = ({title,sub,btn,onBtn,badge}) => (
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:22,flexWrap:"wrap",gap:10}}>
    <div>
      ...
      {sub&&<p style={{color:"#666680",fontSize:12,marginTop:3}}>{sub}</p>}
    </div>
...
function Sidebar({nav,view,setView,onLogout,badge,u}) {
  ...
  return (
    <>
      ...
      <div className={"sidebar-panel"+(open?" open":"")} style={{width:220,background:"#080810",borderRight:"1px solid #151525",display:"flex",flexDirection:"column",position:"fixed",top:0,bottom:0,left:0,zIndex:100}}>
      <div style={{padding:"16px 14px",borderBottom:"1px solid #151525"}}>
      ...
      <div style={{padding:"8px 6px",borderTop:"1px solid #151525"}}>
        <div style={{padding:"9px 10px",background:"#0f0f1e",borderRadius:10,marginBottom:6,display:"flex",alignItems:"center",gap:9}}>
          ...
          <div style={{minWidth:0}}><div style={{fontSize:11,fontWeight:700,color:"#bbb",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{u?.name||"Jan"}</div><div style={{fontSize:9,color:"#555570"}}>{u?.role==="admin"?"Admin":"Klient"}</div></div>
        </div>
        <button onClick={onLogout} style={{width:"100%",background:"none",border:"none",color:"#667788",fontSize:10,cursor:"pointer",fontFamily:"inherit",padding:"3px 0"}}>Wyloguj →</button>
      </div>
    </div>
    </>
  );
        ...
        <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #1a1a2e",borderRadius:20,padding:28}}>
          {[["Email",email,setEmail,"email"],["Hasło",pass,setPass,"password"]].map(([l,v,s,t])=>(
            <div key={l} style={{marginBottom:16}}>
              <div style={{color:"#555570",fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:7}}>{l}</div>
    ...
    <div style={{padding:28}} className="fu">
      ...
      <div className="bottom-grid-3" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
        ...
        <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,overflow:"hidden"}}>
          ...
          {ranked.map((c,i)=>(
            <div key={c.id} className="hr" onClick={()=>onOpen(c.id)} style={{display:"flex",alignItems:"center",padding:"10px 16px",borderBottom:"1px solid #111120",cursor:"pointer",gap:10}}>
              <span style={{color:"#4a4a6a",fontWeight:900,fontSize:11,fontFamily:"'JetBrains Mono',monospace",width:16}}>#{i+1}</span>
        ...
        <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 16px",borderBottom:"1px solid #151525",display:"flex",justifyContent:"space-between"}}>
            ...
            <span style={{color:"#666680",fontSize:10}}>ostatnie 2h</span>
          </div>
          {hotLeads.length===0?<div style={{padding:"30px 16px",textAlign:"center",color:"#555570",fontSize:12}}>Brak gorących leadów</div>:
          hotLeads.map(l=>(
            <div key={l.id} className="hot-row" style={{display:"flex",alignItems:"center",padding:"9px 14px",borderBottom:"1px solid #111120",gap:9}}>
              ...
              <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:"#e0e0e8",fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{l.name}</div><div style={{color:"#555570",fontSize:10}}>{l.cname}</div></div>
              <HotTimer m={l.hot}/>
            </div>
          ))}
        </div>
        ...
        {/* Upcoming events */}
        <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,overflow:"hidden"}}>
          <div style={{padding:"14px 16px",borderBottom:"1px solid #151525"}}><span style={{fontWeight:700,color:"#fff",fontSize:12}}>📅 Najbliższe spotkania</span></div>
          {upcomingEvents.map(ev=>{
            const cl=clients.find(c=>c.id===ev.clientId);
            return (
              <div key={ev.id} style={{display:"flex",padding:"10px 14px",borderBottom:"1px solid #111120",gap:10,alignItems:"center"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{color:"#555570",fontSize:10,marginTop:1}}>{cl?.name} · {ev.date.slice(5)} {ev.time}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{background:"linear-gradient(135deg,#0d0d18,#08080f)",border:"1px solid #151520",borderRadius:18,padding:24,position:"relative"}}>
          ...
          {hovered&&(
            <div style={{position:"absolute",top:20,right:20,background:"#0f0f1e",border:"1px solid #1e1e2e",borderRadius:12,padding:"12px 16px",minWidth:160,pointerEvents:"none"}}>
              ...
              <div style={{color:"#FF6B35",fontSize:20,fontWeight:900,fontFamily:"'JetBrains Mono',monospace"}}>{regionLeads[hovered]||0}</div>
              <div style={{color:"#666680",fontSize:11}}>leadów</div>
            ...
            {Object.entries(regionLeads).sort((a,b)=>b[1]-a[1]).map(([r,l])=>(
              <div key={r} className="hr" onClick={()=>setSelected(selected===r?null:r)} style={{padding:"8px 0",borderBottom:"1px solid #0e0e18",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
                <div style={{flex:1}}>
                  ...
                  <div style={{color:"#555570",fontSize:10}}>{(regionClients[r]||[]).length} klientów</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'JetBrains Mono',monospace",color:"#FF6B35",fontSize:13,fontWeight:700}}>{l}</div>
                  <div style={{color:"#555570",fontSize:9}}>leadów</div>
                </div>
              ...
              {sel.map(c=>(
                <div key={c.id} style={{display:"flex",alignItems:"center",gap:9,padding:"8px 0",borderBottom:"1px solid #0e0e18"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{color:"#555570",fontSize:10}}>{c.city}</div>
                  </div>
                  <div style={{fontFamily:"'JetBrains Mono',monospace",color:"#FF6B35",fontSize:12,fontWeight:700}}>{c.stats.leads}</div>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:18}}>
            ...
            {[["Aktywne regiony",Object.keys(regionLeads).length,"#4ECDC4"],["Łączne leady",Object.values(regionLeads).reduce((a,b)=>a+b,0),"#FF6B35"],["Najlepszy region",Object.entries(regionLeads).sort((a,b)=>b[1]-a[1])[0]?.[0]||"-","#F7C59F"]].map(([l,v,c])=>(
              <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #0e0e18"}}>
                <span style={{color:"#666680",fontSize:12}}>{l}</span>
                <span style={{fontFamily:"'JetBrains Mono',monospace",color:c,fontSize:12,fontWeight:700,textTransform:"capitalize"}}>{v}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            return (
              <div key={ev.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${evColor[ev.type]}25`,borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:14}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{color:"#666680",fontSize:12,marginTop:2}}><span style={{color:cl?.color||"#666",fontWeight:700}}>{cl?.name||"—"}</span> · {ev.date} o {ev.time} · {ev.duration} min</div>
                </div>
          ...
          <div style={{background:"#0d0d18",border:"1px solid #1e1e2e",borderRadius:20,width:"100%",maxWidth:420,padding:28,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            ...
            {[
              ...
              <div key={k} style={{marginBottom:14}}>
                <div style={{color:"#555570",fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>{l}</div>
                {t==="select"?
            ...
            <div style={{marginBottom:18}}>
              <div style={{color:"#555570",fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Typ</div>
          ...
          <div key={c.id} className="hc" onClick={()=>onOpen(c.id)} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:18,padding:20,cursor:"pointer",transition:"all .12s"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{color:"#555570",fontSize:11,marginTop:1}}>{c.city} · {c.email}</div>
              </div>
              <span style={{background:c.plan_key==="pro"?"#FF6B3518":"#141420",color:c.plan_key==="pro"?"#FF6B35":"#5a5a7a",border:`1px solid ${c.plan_key==="pro"?"#FF6B3530":"#1e1e2e"}`,borderRadius:8,padding:"3px 10px",fontSize:10,fontWeight:800,flexShrink:0}}>{c.plan_key==="pro"?"PRO":"FREE"}</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
              {[["Leady",c.stats.leads,"#FF6B35"],["CPL",c.stats.cpl+" zł","#4ECDC4"],["Przychód",fmt(c.stats.revenue)+" zł","#F7C59F"]].map(([l,v,col])=>(
                <div key={l} style={{background:"#08080f",borderRadius:9,padding:"8px 10px"}}>
                  ...
                  <div style={{fontSize:9,color:"#555570",textTransform:"uppercase",letterSpacing:"0.06em",marginTop:2}}>{l}</div>
                </div>
              ))}
            </div>
            <div style={{borderTop:"1px solid #151525",paddingTop:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:"#4a4a6a",fontSize:10}}>Od {c.since} · {c.planPrice} zł/mies</span>
...
function AdminCampaigns({clients}) {
  ...
  return (
    <div style={{padding:28}} className="fu">
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:14,overflow:"hidden",overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 80px 70px 80px 90px",gap:0,padding:"10px 18px",borderBottom:"1px solid #151525",minWidth:600}}>
          {["Kampania","Status","Leady","CPL","Spend","Klient"].map(h=><span key={h} style={{color:"#4a4a6a",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em"}}>{h}</span>)}
        </div>
        {all.map((c,i)=>(
          <div key={c.id} className="hr" style={{display:"grid",gridTemplateColumns:"2fr 1fr 80px 70px 80px 90px",gap:0,padding:"13px 18px",borderBottom:i<all.length-1?"1px solid #111120":"none",alignItems:"center",cursor:"pointer",minWidth:600}}>
            <div><div style={{fontWeight:700,color:"#ddd",fontSize:13}}>{c.name}</div><div style={{color:"#555570",fontSize:10,marginTop:1}}>{c.creative} · od {c.start}</div></div>
            <span style={{background:c.status==="active"?"#4ECDC418":"#1a1a2a",color:c.status==="active"?"#4ECDC4":"#5a5a7a",border:`1px solid ${c.status==="active"?"#4ECDC430":"#1e1e2e"}`,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700,width:"fit-content"}}>{c.status==="active"?"● Aktywna":"⏸ Wstrzymana"}</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#FF6B35",fontWeight:700}}>{c.leads}</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#4ECDC4",fontWeight:700}}>{c.cpl} zł</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#F7C59F",fontWeight:700}}>{fmt(c.spend)} zł</span>
  ...
  return (
    <div style={{padding:28}} className="fu">
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"2px dashed #1a1a2a",borderRadius:16,padding:24,textAlign:"center",marginBottom:20}}>
        ...
        <div style={{color:"#555570",fontSize:12,marginBottom:14}}>Lub kliknij aby wybrać — automatycznie przypisujemy dane do klientów</div>
        <button onClick={()=>showToast("📊 Import CSV z Meta Ads — wkrótce dostępny")} style={{background:`linear-gradient(135deg,${TENANT.primary},#e05020)`,border:"none",color:"#fff",borderRadius:10,padding:"9px 20px",fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>+ Importuj CSV</button>
      </div>
    ...
    <div style={{display:"flex",height:"100vh",maxHeight:"100vh"}} className="fu">
        ...
        {clients.map(c=>(
          <div key={c.id} onClick={()=>setActive(c.id)} style={{padding:"10px 13px",borderBottom:"1px solid #0a0a10",cursor:"pointer",background:active===c.id?"#0f0f1a":"transparent",display:"flex",alignItems:"center",gap:9,transition:"background .1s"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{color:"#4a4a6a",fontSize:10,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.messages[c.messages.length-1]?.text||"—"}</div>
            </div>
          </div>
        ))}
      </div>
      <div style={{flex:1,display:"flex",flexDirection:"column",background:"#060608"}}>
        {cl&&<>
          ...
          <div style={{padding:"10px 14px",borderTop:"1px solid #151525",display:"flex",gap:8,background:"#08080f"}}>
            ...
            <button onClick={send} style={{background:`linear-gradient(135deg,${TENANT.primary},#e05020)`,border:"none",color:"#fff",borderRadius:10,padding:"9px 16px",fontWeight:900,cursor:"pointer",fontSize:14,fontFamily:"inherit"}}>↑</button>
          </div>
        </>}
      </div>
    </div>
      ...
      {clients.map(c=>(
        <div key={c.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:20,marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:800,color:"#fff",fontSize:15}}>{c.name}</div><div style={{color:"#555570",fontSize:11}}>Plan {c.plan} · {fmt(c.planPrice)} zł/mies</div></div>
            <button onClick={()=>showToast("🧾 Generowanie faktury — wkrótce dostępne")} style={{background:"#FF6B3515",border:"1px solid #FF6B3525",color:"#FF6B35",borderRadius:9,padding:"7px 14px",fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Generuj FV →</button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,borderTop:"1px solid #151525",paddingTop:14}}>
            {[["NIP",c.nip],["Email",c.email],["Adres",c.address]].map(([l,v])=>(
              <div key={l}><div style={{color:"#555577",fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4}}>{l}</div><div style={{color:"#5a5a7a",fontSize:11,wordBreak:"break-all"}}>{v}</div></div>
            ))}
          </div>
        </div>
      ))}
        ...
        <div style={{maxWidth:520}}>
          <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:24,marginBottom:16}}>
            ...
            {[["Nazwa firmy","text","name"],["Kolor główny","color","primary"],["Kolor akcentu","color","accent"]].map(([l,t,k])=>(
              <div key={k} style={{marginBottom:16,display:"flex",alignItems:"center",gap:14}}>
                <label style={{color:"#5a5a7a",fontSize:12,fontWeight:600,minWidth:130}}>{l}</label>
          ...
          <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:20}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div><div style={{fontSize:14,fontWeight:800,color:"#fff"}}>{brand.name} Panel</div><div style={{fontSize:10,color:"#555570",letterSpacing:"0.1em",textTransform:"uppercase"}}>Agency Portal</div></div>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,maxWidth:540}}>
            {team.map(m=>(
              <div key={m.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1}}><div style={{fontWeight:700,color:"#e0e0e8",fontSize:13}}>{m.name}</div><div style={{color:"#555570",fontSize:11}}>{m.email}</div></div>
                ...
                <button onClick={()=>showToast("✏️ Edycja — wkrótce")} style={{background:"#0f0f1e",border:"1px solid #1e1e2e",color:"#5a5a7a",borderRadius:7,padding:"4px 8px",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>Edytuj</button>
              </div>
            ))}
            <button onClick={()=>showToast("👥 Dodawanie członka zespołu — wkrótce")} style={{background:"#0d0d18",border:"2px dashed #1a1a2a",color:"#666680",borderRadius:14,padding:"13px 0",fontSize:13,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>+ Dodaj członka zespołu</button>
          </div>
        </div>
      )}
      {tab==="integrations"&&(
          ...
          {intg.map(i=>(
            <div key={i.name} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${i.status==="active"?"#4ECDC425":"#151520"}`,borderRadius:16,padding:18}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
                ...
                <span style={{background:i.status==="active"?"#4ECDC418":"#1a1a2a",color:i.status==="active"?"#4ECDC4":"#5a5a7a",border:`1px solid ${i.status==="active"?"#4ECDC430":"#1e1e2e"}`,borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:700}}>{i.status==="active"?"● Aktywna":"Nieaktywna"}</span>
              </div>
              <div style={{fontWeight:800,color:"#e0e0e8",fontSize:14,marginBottom:4}}>{i.name}</div>
              <div style={{color:"#666680",fontSize:12,marginBottom:14}}>{i.desc}</div>
        ...
        <div className="settings-grid-3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,maxWidth:740}}>
          {[{name:"Starter",price:199,clients:10,features:["Panel klienta","Leady CRM","Raporty podstawowe","Chat"]},{name:"Agency",price:499,clients:50,features:["Wszystko z Starter","White-label","Własna domena","Mapa Polski","Kalendarz","Export CSV"],highlight:true},{name:"Scale",price:999,clients:"∞",features:["Wszystko z Agency","API access","Multi-user team","AI Asystent","SMS powiadomienia","Priorytetowy support"]}].map(p=>(
            <div key={p.name} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`2px solid ${p.highlight?"#FF6B3540":"#151520"}`,borderRadius:18,padding:22,position:"relative"}}>
              ...
              <div style={{fontFamily:"'JetBrains Mono',monospace",color:p.highlight?"#FF6B35":"#ddd",fontSize:28,fontWeight:900,marginBottom:4}}>{p.price} zł<span style={{fontSize:12,color:"#666680",fontWeight:400}}>/mies</span></div>
              <div style={{color:"#666680",fontSize:12,marginBottom:16}}>Do {p.clients} klientów</div>
              <div style={{borderTop:"1px solid #151525",paddingTop:14}}>
        ...
        <div style={{maxWidth:560}}>
          <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:22}}>
            ...
            <div style={{marginBottom:14}}>
              <div style={{color:"#555570",fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Live API Key</div>
              <div style={{background:"#08080f",border:"1px solid #151525",borderRadius:9,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#666680",fontSize:11}}>hg_live_•••••••••••••••••••••••••••</span>
                <button onClick={()=>showToast("📋 Klucz API skopiowany")} style={{background:"#FF6B3515",border:"1px solid #FF6B3525",color:"#FF6B35",borderRadius:6,padding:"3px 8px",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:700}}>Kopiuj</button>
              </div>
            </div>
            <div style={{background:"#FF6B3508",border:"1px solid #FF6B3520",borderRadius:10,padding:"12px 14px",marginTop:16}}>
              ...
              <div style={{color:"#5a5a7a",fontSize:11,lineHeight:1.6}}>REST API dostępne w planach Scale i Enterprise. Endpointy: /clients, /leads, /campaigns, /reports. Rate limit: 1000 req/h.</div>
              <div style={{marginTop:10,color:"#666680",fontSize:11,fontFamily:"'JetBrains Mono',monospace",background:"#0a0a0f",borderRadius:8,padding:"8px 12px"}}>GET https://api.hardgain.pl/v1/clients<br/>Authorization: Bearer {"{"}"api_key{"}"}</div>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{padding:28}} className="fu">
        ...
        <div>{live.campaigns.map(cp=>(
          <div key={cp.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:20,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
              <div><div style={{fontWeight:800,color:"#fff",fontSize:15}}>{cp.name}</div><div style={{color:"#555570",fontSize:12,marginTop:2}}>Od {cp.start} · {cp.creative}</div></div>
              <span style={{background:cp.status==="active"?"#4ECDC418":"#1a1a2a",color:cp.status==="active"?"#4ECDC4":"#5a5a7a",borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:700}}>{cp.status==="active"?"● Aktywna":"⏸ Wstrzymana"}</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
              {[["Leady",cp.leads,"#FF6B35"],["CPL",cp.cpl+" zł","#4ECDC4"],["Spend",fmt(cp.spend)+" zł","#F7C59F"],["Budżet/dz",cp.budget+" zł","#888"]].map(([l,v,c])=>(
                <div key={l} style={{background:"#08080f",borderRadius:10,padding:"10px 12px"}}><div style={{fontWeight:800,color:c,fontFamily:"'JetBrains Mono',monospace",fontSize:16}}>{v}</div><div style={{color:"#4a4a6a",fontSize:9,textTransform:"uppercase",letterSpacing:"0.06em",marginTop:3}}>{l}</div></div>
              ))}
            </div>
          </div>
        ))}</div>
      ...
      {tab==="leads"&&(
        <div>{live.leads.map(l=>(
          <div key={l.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${l.hot<=60?"#FF6B3520":"#151520"}`,borderRadius:12,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
            <div style={{width:32,height:32,background:"#FF6B3515",borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",color:"#FF6B35",fontWeight:900,fontSize:12,flexShrink:0}}>{l.name.charAt(0)}</div>
            <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,color:"#e0e0e8",fontSize:13}}>{l.name}</div><div style={{color:"#555570",fontSize:11,marginTop:1}}>{l.phone} · {l.campaign}</div></div>
            <HotTimer m={l.hot}/>
            <LBadge s={l.status}/>
                ...
                <div key={k} style={{marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    ...
                    <div style={{display:"flex",gap:10}}>
                      ...
                      <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#fff",fontSize:13,fontWeight:700}}>{v}</span>
                    </div>
                  </div>
      ...
      {tab==="creatives"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
          {live.creatives.length===0?<div style={{color:"#555570",fontSize:13}}>Brak kreacji.</div>:
          live.creatives.map(cr=>(
            <div key={cr.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${cr.status==="pending_approval"?"#F7C59F30":"#151520"}`,borderRadius:14,padding:16}}>
              ...
              <div style={{color:"#555570",fontSize:10,marginBottom:10}}>{cr.campaign}</div>
      ...
      {tab==="schedule"&&(
        <div>
          ...
          {clientEvents.length===0?<div style={{textAlign:"center",color:"#4a4a6a",padding:"40px 0"}}>Brak spotkań. <button onClick={()=>{}} style={{background:"none",border:"none",color:"#FF6B35",cursor:"pointer",fontFamily:"inherit",fontSize:12}}>Zaplanuj →</button></div>:
          clientEvents.sort((a,b)=>a.date.localeCompare(b.date)).map(ev=>(
            <div key={ev.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${evColor[ev.type]}25`,borderRadius:14,padding:"14px 18px",display:"flex",gap:14,alignItems:"center",marginBottom:8}}>
              ...
              <div style={{flex:1}}><div style={{fontWeight:800,color:"#fff",fontSize:14}}>{ev.title}</div><div style={{color:"#555570",fontSize:12,marginTop:2}}>{ev.date} o {ev.time} · {ev.duration} min</div></div>
      ...
      {tab==="tickets"&&(
        <div>{live.tickets.length===0?<div style={{textAlign:"center",color:"#4a4a6a",padding:"40px 0"}}>Brak zgłoszeń 🎉</div>:
        live.tickets.map(t=>(
          <div key={t.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:12,padding:"13px 18px",display:"flex",gap:12,alignItems:"center",marginBottom:8}}>
            <div style={{flex:1}}><div style={{fontWeight:700,color:"#e0e0e8"}}>{t.title}</div><div style={{color:"#555570",fontSize:11,marginTop:2}}>{t.date}</div></div>
...
function ProLock({label,onUpgrade}) {
  return (
    <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:60,textAlign:"center"}}>
      ...
      <div style={{color:"#666680",fontSize:13,marginBottom:20}}>Odblokuj w planie Pro za 99 zł/mies</div>
...
function ClientCampaigns({c,isPro,onUpgrade}) {
  return (
    <div style={{position:"relative"}}>
      ...
      {c.campaigns.map(cp=>(
        <div key={cp.id} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,padding:20,marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}><div><div style={{fontWeight:800,color:"#fff",fontSize:15}}>{cp.name}</div><div style={{color:"#555570",fontSize:12,marginTop:2}}>Od {cp.start}</div></div><span style={{background:cp.status==="active"?"#4ECDC418":"#1a1a2a",color:cp.status==="active"?"#4ECDC4":"#5a5a7a",borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:700}}>{cp.status==="active"?"● Aktywna":"⏸"}</span></div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {[["Leady",cp.leads,"#FF6B35"],["CPL",cp.cpl+" zł","#4ECDC4"],["Spend",`${cp.spend} zł`,"#F7C59F"],["Budżet",cp.budget+" zł/dz","#888"]].map(([l,v,col])=>(
              <div key={l} style={{background:"#08080f",borderRadius:9,padding:"9px 11px"}}><div style={{fontWeight:800,color:col,fontFamily:"'JetBrains Mono',monospace",fontSize:15}}>{v}</div><div style={{color:"#4a4a6a",fontSize:9,textTransform:"uppercase",letterSpacing:"0.06em",marginTop:2}}>{l}</div></div>
            ))}
          </div>
        </div>
      ))}
...
function ClientReports({c,isPro,onUpgrade}) {
  ...
  return <div style={{textAlign:"center",color:"#555570",padding:"40px 0",fontSize:14}}>Raporty zostaną wygenerowane automatycznie do 3. dnia miesiąca.</div>;
}
  ...
  return (
    <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:16,display:"flex",flexDirection:"column",height:400}}>
      ...
      <div style={{padding:"9px 12px",borderTop:"1px solid #151525",display:"flex",gap:8}}>
        ...
        <button onClick={send} style={{background:`linear-gradient(135deg,${TENANT.primary},#e05020)`,border:"none",color:"#fff",borderRadius:9,padding:"8px 15px",fontWeight:900,cursor:"pointer",fontSize:14,fontFamily:"inherit"}}>↑</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:14,padding:"14px 18px",marginBottom:18,display:"flex",alignItems:"center",gap:14}}>
        ...
        <span style={{color:TENANT.primary,fontWeight:900,fontSize:14,fontFamily:"'JetBrains Mono',monospace"}}>{done}/{steps.length}</span>
              ...
              <div key={m.id} onClick={!m.free&&!isPro?onUpgrade:undefined} style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:`1px solid ${!m.free&&!isPro?"#0f0f18":"#151520"}`,borderRadius:12,padding:14,cursor:!m.free&&!isPro?"pointer":"default",opacity:!m.free&&!isPro?.5:1}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{color:"#555570",fontSize:11,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                      {m.dur}
...
function CampaignOrder() {
  ...
  if(step===3) return (
    <div style={{textAlign:"center",paddingTop:40}}>
      ...
      <div style={{color:"#666680",fontSize:13,marginBottom:20}}>Odezwiemy się w ciągu 24h z planem kampanii.</div>
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #151520",borderRadius:18,padding:26,maxWidth:520}}>
        {step===1&&<>
          ...
          {[["Cel kampanii","goal","text","np. Zbieranie leadów na pakiet treningowy"],["Miesięczny budżet (zł)","budget","number","np. 1500"]].map(([l,k,t,ph])=>(
            <div key={k} style={{marginBottom:16}}><div style={{color:"#555570",fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:7}}>{l}</div><input type={t} value={form[k]} onChange={e=>set(k,e.target.value)} placeholder={ph} style={{width:"100%",background:"#08080f",border:"1px solid #151525",borderRadius:10,padding:"11px 14px",color:"#ddd",fontSize:13,outline:"none"}}/></div>
          ))}
...
function TicketForm() {
  ...
  if(sent) return <div style={{textAlign:"center",paddingTop:60}}><div style={{fontSize:48,marginBottom:14}}>✅</div><div style={{fontWeight:900,color:"#fff",fontSize:20,marginBottom:8}}>Zgłoszenie wysłane!</div><div style={{color:"#666680",fontSize:13}}>Odpiszemy w 24h roboczych.</div><button onClick={()=>{setSent(false);setTitle("");setDesc("");}} style={{marginTop:20,background:`linear-gradient(135deg,${TENANT.primary},#e05020)`,border:"none",color:"#fff",borderRadius:10,padding:"10px 20px",fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Nowe zgłoszenie</button></div>;
  return (
    <div>
      ...
      <div style={{background:"linear-gradient(135deg,#0d0d18,#0a0a12)",border:"1px solid #1e1e2e",borderRadius:22,width:"100%",maxWidth:480,padding:32,boxShadow:`0 24px 80px #000`,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:24}}>
          {["✓ Pełne dane leadów — imię, telefon, status","✓ Wykresy CPL i trendów w czasie","✓ Raporty miesięczne z historią","✓ Zatwierdzanie kreacji reklamowych","✓ Chat z agencją bez limitu","✓ Eksport leadów do CSV","✓ Zamówienia nowych kampanii","✓ Powiadomienia push przy nowym leadzie"].map(f=>(
            <div key={f} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:"1px solid #151525",color:"#666",fontSize:13}}>{f}</div>
          ))}
        </div>
        <div style={{display:"flex",gap:10}}>
...
export default function App() {
  ...
  return <>
    <Toast/>
    {!user?<Login onLogin={setUser}/>:user.role==="admin"?<AdminApp user={user} onLogout={()=>setUser(null)}/>:<ClientApp user={user} onLogout={()=>setUser(null)}/>}
  </>;
}
